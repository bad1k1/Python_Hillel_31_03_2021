...
def generate_email(names, domains):
    rand_email = f"{random.choice(['king','miller','kean'])}.{random.randint(100,999)}@{''.join(random.choice(string.ascii_letters + string.digits) for _ in range(5, 7))}.{random.choice(['net','com','ua'])}"
    return rand_email
#
#
